...
En general, en R los números son tratados como números reales de dos decimales. Si no se especifícase con `L` el tipo de clase, el resultado hubiera sido `5.00`.
...
Las matrices son vectores con atributo "dimensión": la cantidad de filas y la cantidad de columnas
...
El `factor` se puede pensar como un vector de `Integer` en el que cada item tiene su propia etiqueta. Los objetos de este tipo suelen utilizarse para representar información categórica.
...
# así accedemos al primer elemento
my_vec[1]

# o a un conjunto de ellos
my_vec[1:3] # 1.secuencia de integers

# 2.vector de integers
my_vec[c(1,3)] 
...
# seleccionamos un elemento a partir de su nombre literal
x$domicilio

# también lo podríamos haber hecho con su valor ordinal
x[[1]]

x$consumo

# no hace falta recordar el orden exacto 
x[['consumo']]


# también podemos acceder al tercer elemento del primero en la lista
x[[c(1,3)]]
...
# verificamos la cantidad de días transcurridos de una fecha a otra
