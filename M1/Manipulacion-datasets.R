...
# Seleccionamos las observaciones de las temporadas 1 y 2
idx <- 1
for (t in seq(2)){
  # Subset data
  key <- paste("temporada", idx, sep = "")
  airquality_df <- subset(
    airquality, Month %in% temporadas[[key]],
    select = c(Temp, Wind)
    )
...
# Moda
calcula_moda <- function(seq.val) {
  return(as.numeric(names(which.max(table(seq.val)))))
}

temporadas[["Mod.Temp1"]] <- calcula_moda(temporadas$airquality1$Temp)
temporadas[["Mod.Temp2"]] <- calcula_moda(temporadas$airquality2$Temp)